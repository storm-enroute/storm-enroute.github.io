...
permalink: /coroutines/docs/0.7/datatypes/index.html
...
It is represented with the type `Coroutine.Instance[Y, R]`.
Consider the following coroutine `twice`:

    val twice = coroutine { (x: Int) => 2 * x }

The coroutine instance type is `Coroutine.Instance[Nothing, Int]`:

    val c: Coroutine.Instance[Nothing, Int] = call(twice(7))
...
[API](http://storm-enroute.com/apidocs/coroutines/0.7/api/)
